...
class JoyStick(
    ...
    private var resolvedCenterX = if (centerX >= 0f) centerX else gctx.metrics.width + centerX
    private var resolvedCenterY = if (centerY >= 0f) centerY else gctx.metrics.height + centerY
    ...
    fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                isVisible = true
                resolvedCenterX = pt.x
                resolvedCenterY = pt.y
                bgRect.set(
                    resolvedCenterX - bgRadius,
                    resolvedCenterY - bgRadius,
                    resolvedCenterX + bgRadius,
            ...
            MotionEvent.ACTION_MOVE -> {
                val dx = pt.x - resolvedCenterX
                val dy = pt.y - resolvedCenterY
